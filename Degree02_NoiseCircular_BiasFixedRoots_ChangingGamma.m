...
folder_name='Results/Degree02_NoiseCircular_BiasFixedRoots_Changing_Gamma'; %Name for the results folder: it should be named after the kind of test performed
...
sigma_a=1 %1 %.5 %.3 %.1 %.05 %.01; % variance of the noise on coefficients
...
Gamma = Sigma(1:N,1:N)
disp(strcat(num2str(max(eig(Gamma))),' <= ',num2str(1.78086^2/N)))
% Generate roots
% r=[scale*(2*rand(N,1)-1)+scale*1i*(2*rand(N,1)-1)];
r = [0.7500 + 1.0944i; -1.4124 - 0.8093i];
...
for k=1:K
    ...
    r_n(:,k)=r_curr(order_roots_permutations_hermitian(r_curr,r)); %Save roots ordered w.r.t. original roots
    err_n(:,k)=r_n(:,k)-r;
